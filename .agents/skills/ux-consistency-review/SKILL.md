...
- **Lead with the answer (Pirámide de Minto)**: la vista debe estructurarse de lo macro a lo micro. Arriba/primero: el insight o el desvío principal (¿cumplimos el Goal? ¿dónde está el gap?). Después: los drivers (por LOB / país / palanca). Al final: el detalle granular o transaccional. Es un hallazgo si el usuario tiene que escanear una tabla densa o cruzar varias columnas para deducir el "titular" — ese titular debería estar explícito (un KPI grande, un texto de resumen, un semáforo). También: **¿la altitud coincide con el lector?** C-level = solo titular + 2-3 drivers; gerente comercial = drivers + su cartera; analista financiero = todo el detalle. Una vista pensada para C-level que abre con una tabla de 25 líneas está en la altitud equivocada.
...
- **Cada gráfico responde a UN propósito analítico (Zelazny)**: tendencia, comparación, composición, relación o distribución. Antes de mirar el tipo, preguntar qué mensaje quiere transmitir el chart; el tipo se deduce de ahí. Es un hallazgo un chart cuyo propósito no está claro, o que mezcla dos (ej. una barra apilada que quiere mostrar tendencia *y* composición a la vez y no logra ninguna). Prohibido: torta/dona 3D, y **exceso de gauges/medidores** (un dashboard con 6 gauges es ruido — un gauge se justifica solo para 1-2 KPIs "de un vistazo, ¿estamos bien?").
...
- **Carga cognitiva de conciliación Actuals vs Budget/Forecast/otra fuente**: en las tablas comparativas (P&L Baseline/Goal, Managerial vs Accounting, PxQ), ¿se lee la **desviación** de un vistazo, o el usuario tiene que restar mentalmente dos columnas? La Δ y la Δ% deberían estar calculadas y visibles, con color+signo, no dejadas al lector. Señalar tablas donde el dato importante (el gap) esté implícito. Patrón claro de **lectura vs input**: si una vista mezcla celdas editables (planning) con celdas de solo-lectura (reporting) sin distinción visual, es un hallazgo.
