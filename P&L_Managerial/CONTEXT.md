...
- **`revenue_gd_builder.py`** — herramienta aparte, NO parte del refresh regular. Corre la query "revenue por GD" (modelo de revenue B2B: `bi_transactional_fact_*` + `lake.b2b_rev_pnl_sales_detail` + `lake.b2b_b2b2c_revenue_pnl_model`, basis `gestion_date`) para los meses cerrados de FY27, la mapea a `METRIC_COLS` y sube **`revenue_gd.json`** (fileId `1gLerphm9YJZ_-aKW7J566a9QcNbIQOCz`, misma carpeta de Drive) — lo consume la landing para el modo "Actuals GD vs Revenue" de Managerial vs Accounting. **El mapeo de las ~30 líneas P&L y los signos de las líneas de costo están en BORRADOR** (el header del script tiene la tabla). 6 métricas no tienen fuente en la query y salen 0 (`income_from_outsourced_services`, `white_labels_api`, `back_end_incentives`, `intercompany_usd`, `operations`, `vendor_commissions`). Métricas extra que sí trae la query y no están en `METRIC_COLS`: `rev_gb`, `revenue_margin`, `agency_fee`, `bad_debt`. Verificado Abr-Ago 2026 (Total B2B): GB reconcilia ~0.1% con el `ac` gestional, NR ~1-2%, FVM ~5% (Abr-Jun) / OK (Jul-Ago); el split up_front/fees/discounts difiere estructuralmente entre los dos modelos. **Análisis completo de por qué difieren las dos queries** (fuentes, signos, country_factor, grain, mapeo país/canal, líneas faltantes) en `reconciliacion_actuals_gd_vs_revenue.md` — también resumido en un `<details>` del header de la vista.
- **`dashboard.html`** — frontend GAS: Managerial View B2B2C y B2B (FVM, waterfall, evolución, palancas, NR Bridge, PxQ) + vista **Managerial vs Accounting**. Sin vistas contables (esas quedaron en P&L Accounting). La vista Managerial vs Accounting tiene un selector de modo (`_vsAccMode`): **"Managerial vs Accounting"** (`mgracc` — 3 tablas B2B2C/MAY/MIN, gestional `ac`/`ac_ri` vs `actuals.json` canónico, mapeo `VSACC_MAP`) y **"Actuals GD vs Revenue"** (`planrev` — cuadro resumen país×canal NR+FVM con selector de mes, + detalle P&L en 3 tablas — Total B2B / MAY (API) / MIN (AFF): gestional `ac` GD vs `revenue_gd.json`). Ambos modos comparten los chips país/producto y el selector de Unidades (M/K/Enteros, `_numScale` global). El modo `planrev` no depende de `getVsAccounting` (pipeline propio).
...
- **`check_b2b_query_drift.py [YYYY-MM]`** — herramienta de diagnóstico (2026-09-20), NO parte del refresh regular. Compara los totales GB/NR/FVM por país que devuelven las queries B2B GD/RI de este módulo contra las de `Daily_Dashboard/daily_sync.py` (la versión "oficial" — ver gotchas abajo, ya divergieron 3 veces) para un mes dado. Extrae `build_b2b_gd_query`/`build_b2b_ri_query` de cada script vía AST (solo esos nodos — nunca ejecuta el resto del archivo, así que no dispara ningún upload). Corta con exit code 1 y detalle por país/métrica si algo diverge más de 0.5%. Correrlo después de tocar cualquiera de las dos queries, o periódicamente para pescar un drift antes que un usuario.
...
- **`RUTA_ENV`** (`actuals_gestional_upload.py:44` **y también** `revenue_gd_builder.py:67`, cada uno con su propia copia literal) apunta a `C:\Users\diego.bracco\Proyectos IA\envs\.env` — hardcodeado a la máquina de Diego. Si otra persona corre cualquiera de los dos scripts, hay que cambiar la ruta a mano en el que corresponda (no está cubierto por el checklist de onboarding del `CLAUDE.md` raíz, que solo menciona `Daily_Dashboard`).
...
- **RESUELTO (auditoría 2026-09-20):** `fetch()` ahora aborta (`sys.exit`) si cualquier query del Datalake devuelve 0 filas — antes el script seguía y subía igual el JSON con esa sección vacía, pisando el archivo bueno de producción. Además, después de armar `ac_b2b2c`/`ac_may`/`ac_min` se chequea (WARN, no aborta) que cada mes de `actual_months` tenga al menos 1 fila — cubre el caso de una query que vuelve con filas pero le falta un mes puntual (join que no matcheó). Sigue sin haber backup/versionado explícito antes del overwrite en Drive (mitigado parcialmente por el historial nativo de revisiones de Drive).
- **RESUELTO (auditoría 2026-09-20) — tercer drift entre `build_b2b_ri_query` y `daily_sync.py`:** desde el fix del 2026-09-07 (ver gotcha arriba), `daily_sync.py` recibió un ajuste posterior no propagado acá (commit `8176cfb`, "checkin_date solo para API+Hoteles, resto por recognition_date") — la condición pasó de `parent_channel='API'` (todo API) a `parent_channel='API' AND buy_type_code='Hoteles'`. `actuals_gestional_upload.py` había quedado con la versión vieja (todo API), y además tenía un filtro `fh.lob_gestion IN ('stg__sales_b2bnohoteldo','stg_sales__b2bhoteldo')` en el WHERE de RI que **no corresponde ahí** — ese filtro sí existe en `daily_sync.py` pero solo en la query GD, nunca en la RI. Encontrado con `check_b2b_query_drift.py` (ver Stack): Brasil RI GB/FVM divergían ~0.7-0.9% entre los dos scripts para ago-2026. Fix: se alinearon las 6 apariciones de la condición de fecha (SELECT anio_ri/mes_ri, WHERE, GROUP BY ×3) a `parent_channel='API' AND buy_type_code='Hoteles'` y se sacó el filtro `lob_gestion` de la query RI. **Verificado 2026-09-20:** re-corrido `check_b2b_query_drift.py` para ago-2026 → GD y RI coinciden dentro de 0.5% en los 10 países. Pipeline re-corrido con el fix, JSON actualizado en Drive.
